...
router.get("/", 
	...
	console.log('[api] GET /products query ->', query)
	try {
		let filter = {}

		if (query.gender) {
			const category = query.gender === 'female' ? 'women' : query.gender === 'male' ? 'men' : query.gender
		...
		let products 
		if (query.new === "true") {
			products = await Product.find(filter).sort({ createdAt: -1 }).limit(limit || 5)
		} else {
			products = await Product.find(filter).limit(limit).skip(skip)
		}
